...
#define MAIX_PRODUCT_NAME "SYSU_AIOTOS"
...
static void maix_runtime_sync_state(void)
{
    g_state.uptime_ms = rt_tick_get() * (1000 / RT_TICK_PER_SECOND);
}

static void maix_runtime_probe_services(void)
{
    maix_runtime_fs_init(&g_profile, &g_state);
    maix_runtime_python_init(&g_profile, &g_state);
    maix_runtime_model_refresh(&g_profile, &g_state);
}

static void maix_runtime_reset(void)
{
    maix_runtime_load_defaults(&g_profile);
    maix_board_fill_profile(&g_profile);

    maix_runtime_sync_state();
    g_state.heartbeat_count = 0;
    g_state.heap_total = 0;
    g_state.heap_used = 0;
    g_state.heap_peak = 0;
    g_state.storage_backend = "none";
    g_state.python_backend = "none";
    g_state.model_backend = "none";
    g_state.active_script = "/maixapp/apps/sysu_aiotos_demo/main.py";
    g_state.model_slot = "external:/models";
    g_state.boot_status = MAIX_BOOT_NOT_RUN;

    maix_runtime_probe_services();
}

static const char *maix_boot_status_name(maix_boot_status_t status)
{
    switch (status)
    {
    case MAIX_BOOT_PENDING:
        return "pending";
    case MAIX_BOOT_OK:
        return "ok";
    case MAIX_BOOT_BLOCKED:
        return "blocked";
    case MAIX_BOOT_FAILED:
        return "failed";
    case MAIX_BOOT_NOT_RUN:
    default:
        return "not-run";
    }
}
...
static void maix_runtime_print_profile(void)
{
    ...
    rt_kprintf("[RT] storage_backend=%s python_backend=%s model_backend=%s\n",
               g_state.storage_backend,
               g_state.python_backend,
               g_state.model_backend);
    ...
    rt_kprintf("[NEXT] integrate mounted DFS media and real inference backend\n");
}

static void maix_runtime_print_state(void)
{
    maix_runtime_sync_state();
    rt_kprintf("[STATE] uptime_ms=%lu heartbeat=%lu boot=%s script=%s model_slot=%s\n",
               (unsigned long)g_state.uptime_ms,
               g_state.heartbeat_count,
               maix_boot_status_name(g_state.boot_status),
               g_state.active_script,
               g_state.model_slot);
}

static void maix_runtime_boot_pipeline(void)
{
    maix_runtime_fs_print_manifest();
    maix_runtime_python_run_boot(&g_state);
    maix_runtime_model_refresh(&g_profile, &g_state);
    maix_runtime_model_print_status();
}

static void maix_runtime_print_diag(void)
{
    maix_runtime_probe_services();
...
static void maix_runtime_mark_boot_status(void)
{
    if (g_state.boot_status == MAIX_BOOT_NOT_RUN)
    {
        g_state.boot_status = MAIX_BOOT_PENDING;
    }
}
...
int maix_runtime_main(void)
{
    ...
    maix_runtime_reset();
    maix_runtime_print_profile();
    maix_runtime_boot_pipeline();
    maix_runtime_mark_boot_status();
...
MSH_CMD_EXPORT(maix_info, print SYSU_AIOTOS runtime profile);

static void maix_state(void)
{
    maix_runtime_print_state();
}
MSH_CMD_EXPORT(maix_state, print SYSU_AIOTOS runtime state);

static void maix_fs(void)
{
    maix_runtime_probe_services();
    maix_runtime_fs_print_manifest();
}
MSH_CMD_EXPORT(maix_fs, list bundled runtime files);

static void maix_py(void)
{
    maix_runtime_probe_services();
    maix_runtime_python_print_status();
}
MSH_CMD_EXPORT(maix_py, print Python backend status);

static void maix_model(void)
{
    maix_runtime_probe_services();
    maix_runtime_model_print_status();
}
MSH_CMD_EXPORT(maix_model, print model backend status);

static void maix_diag(void)
{
    maix_runtime_print_diag();
}
MSH_CMD_EXPORT(maix_diag, print consolidated SYSU_AIOTOS diagnostics);
#endif
